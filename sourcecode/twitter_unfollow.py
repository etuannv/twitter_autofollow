...
def unfollowUser(username):
    ...
    xpath = ".//a[contains(@href,'{}/photo')]/following::div[1]//div[contains(@data-testid,'-unfollow')]".format(username)
    # tag = browser.findByXpath(".//input[@type='submit' and @value='unfollow']", 3)
    ...
    if tag:
        ...
        time.sleep(1)
        logging.info("{} is unfollowed".format(username))

        return True
        # elif:
        #     tag = browser.findByXpath(".//div[@class='ProfileNav']//span[text()='unfollowing']/parent::button")
    else:
        logging.info("{} could not be unfollowed".format(username))
        return False
...
def main(argv):
    ...
    DoneFile = os.path.join(CurrentPath, 'unfollow-done.txt')
    CookiePath = os.path.join(CurrentPath, 'cookies.pkl')
    browser = None
    
    print('==================== TWITTER AUTO unfollow ====================')
    ...
    # Read csv file for users that need to be unfollow
